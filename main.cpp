...
private:
    ...
    int isVertexEar(int vectorLength, const vector<Point> &pointsVector)
    {
        return (isVertexInsideNewPoly(vectorLength,pointsVector) && !isEdgeIntersect(vectorLength, pointsVector));
    }

    int isEdgeIntersect(int vectorLength, const vector<Point> &pointsVector)
    {
        Point auxPoint = pointsVector[vectorLength];
        vector<Point> aux;

        for (size_t i = 0; i < pointsVector.size(); i++)
            if (i != vectorLength)
                aux.push_back(pointsVector[i]);

        int c = 0, cnt = aux.size(), prev = (cnt + (vectorLength - 1)) % cnt, next = vectorLength % cnt;
    ...
    int isVertexInsideNewPoly(int vectorLength, const vector<Point> &pointsVector)
    {
        Point auxPoint = pointsVector[vectorLength];
        vector<Point> aux;
        ...
        for (size_t i = 0, j = aux.size() - 1; i < aux.size(); j = i++)
        {
            ...
            if ((((aux[i].y <= auxPoint.y) && (auxPoint.y < aux[j].y)) ||
                    ((aux[j].y <= auxPoint.y) && (auxPoint.y < aux[i].y)))&&
                    (auxPoint.x > (aux[j].x - aux[i].x) *(auxPoint.y - aux[i].y)/ (aux[j].y - aux[i].y) + aux[i].x))
                c = !c;
    ...
    int orientation(Point point1,Point point2,Point point3)
    {
        ...
        if(value==0) return 0; ///collinearity
        return (value>0)? 1: 2; ///1-for right, 2 for left
    }
        ...
        for (size_t t = aux.size() - 1, i = 0, j = 1; i < aux.size(); t = i++, j = (i + 1) % aux.size())
        {
            if (aux.size() == 3)
            {
                ...
                if(test!=1)
                {
                    ...
                    if(orientation(aux[0],aux[1],pLocate)== 0 ||
                        orientation(aux[1],aux[2],pLocate) == 0 ||
                        orientation(aux[2],aux[0],pLocate) == 0 )
                    {
            ...
            if (isVertexEar(i, aux))
            {
                ...
                if(test!=1)
                {
                    ...
                    if(orientation(aux[t],aux[i],pLocate)== 0 ||
                        orientation(aux[i],aux[j],pLocate) == 0 ||
                        orientation(aux[j],aux[t],pLocate) == 0 )
                    {
                        printf("The point is inside of the triangle formed by the points: (%.1f, %.1f), (%.1f, %.1f ), (%.1f, %.1f)\n",
